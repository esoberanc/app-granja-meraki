...
def api_resumen_ia():
    try:
        ...
        # Abre la hoja de cálculo y obtiene los datos
        spreadsheet = client.open_by_key("1tNx0hjnQzdUKoBvTmIsb9y3PaL3GYYNF3_bMDIIfgRA")
        sheet = spreadsheet.worksheet("Hoja1")
        data = sheet.get_all_records()

        # Carga los últimos 1000 registros como DataFrame
        df = pd.DataFrame(data)
        df = df.tail(1000)

        if df.empty:
            raise Exception("No hay datos suficientes en Google Sheets")

        # Generar un resumen estadístico simple como contexto
...
Resumen:"""
        ...
        openai.api_key = open("secrets/openai_key.txt").read().strip()
        ...
        return jsonify({"resumen": "Error al generar el resumen. Verifica la conexión con Google Sheets o la clave de OpenAI."})
